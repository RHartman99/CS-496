...
let extend_env_rec_list : string -> string -> Ast.expr -> env ea_result =
  fun l env ->
  match l with
  | h::t -> extend_env_rec h
  | [] -> Ok(EmptyEnv)

let build_env_rec_list = fun l env ->
  match l with
  | (v, par, body)::t -> extend_env_rec v 
  | []
  ...
  | ExtendEnvRec(decs,tail) ->
    (match decs with
     | (v, par, body)::t -> if v = id 
                            ...
                            else apply_env id (ExtendEnvRec(t, tail))
     | [] -> apply_env id tail) 
