(* Names: Ryan Hartman and Andrew Pantera *)
...
let extend_env_rec : (string*string*Ast.expr) list -> env ea_result = fun decs env  ->
  Ok (ExtendEnvRec(decs,env))

let rec find_in_decs : string -> (string*string*Ast.expr) list -> env -> exp_val option = fun id decs env  ->
  match decs with
  | (v, par, body)::t ->  if v = id
                          then Some(ProcVal(par,body,env))
                          else find_in_decs id t env
  | [] -> None
...
let rec apply_env : string -> exp_val ea_result =
  ...
  | ExtendEnvRec(decs,tail) ->
    (match (find_in_decs id decs env) with
     | Some(ProcVal(par,body,_)) -> Ok( ProcVal(par, body, env) )
